        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalAnnouncements = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAnnouncements">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Announcement</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AnnouncementId" type="text" placeholder="Enter AnnouncementId" v-model="announcementToAdd.announcementId"></base-input>
  <base-input label="Message" type="text" placeholder="Enter Message" v-model="announcementToAdd.message"></base-input>
  <base-input label="Date" type="text" placeholder="Enter Date" v-model="announcementToAdd.date"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="announcements" :row-key="record => record.AnnouncementId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AnnouncementPictureView :announcements="announcements" />
          </div>
...
import AnnouncementPictureView from './AnnouncementPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'announcementId',
          visible: true,
          scopedSlots: { customRender: 'announcementId' },
          sorter: true
          //sorter: (a, b) => a.announcementId - b.announcementId,
          //sorter: (a, b) => a.announcementId.localeCompare(b.announcementId),
        },
        {
          title: 'Message',
		dataIndex: 'message',
      ...
      sortBy: 'announcementId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAnnouncementsTable() {
      ...
      let announcementsTableData = [];
      for (let i = 0; i < this.announcements.length; i++) {
        announcementsTableData.push({
          id: i,
          announcementId: this.announcements[i].announcementId,
          year: this.announcements[i].year,
          date: this.announcements[i].date,
          competitionId: this.announcements[i].competitionId,
          announcementId: this.announcements[i].announcementId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-announcements',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAnnouncementsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalAnnouncements = false;

      const currentDate = new Date().getTime();
      this.announcementToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.announcementToAdd);
      console.log(jsonData);
      
      const res = await AnnouncementService.addAnnouncement(jsonData);
