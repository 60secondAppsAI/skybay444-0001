        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Loyaltys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalLoyaltys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLoyaltys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Loyalty</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LoyaltyId" type="text" placeholder="Enter LoyaltyId" v-model="loyaltyToAdd.loyaltyId"></base-input>
  <base-input label="Points" type="text" placeholder="Enter Points" v-model="loyaltyToAdd.points"></base-input>
  <base-input label="Tier" type="text" placeholder="Enter Tier" v-model="loyaltyToAdd.tier"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="loyaltys" :row-key="record => record.LoyaltyId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <LoyaltyPictureView :loyaltys="loyaltys" />
          </div>
...
import LoyaltyPictureView from './LoyaltyPictureView.vue';


const loyaltysColumns = [
  "loyaltyId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'loyaltyId',
          visible: true,
          scopedSlots: { customRender: 'loyaltyId' },
          sorter: true
          //sorter: (a, b) => a.loyaltyId - b.loyaltyId,
          //sorter: (a, b) => a.loyaltyId.localeCompare(b.loyaltyId),
        },
        {
          title: 'Points',
		dataIndex: 'points',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} loyaltys`,
      },
      ...
      loyaltysTable: {
        columns: [...loyaltysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'loyaltyId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLoyaltysTable() {
      ...
      let loyaltysTableData = [];
      for (let i = 0; i < this.loyaltys.length; i++) {
        loyaltysTableData.push({
          id: i,
          loyaltyId: this.loyaltys[i].loyaltyId,
          year: this.loyaltys[i].year,
          date: this.loyaltys[i].date,
          competitionId: this.loyaltys[i].competitionId,
          loyaltyId: this.loyaltys[i].loyaltyId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-loyaltys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLoyaltysChanged', this.searchQuery);
		//this.renderLoyaltysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalLoyaltys = false;

      const currentDate = new Date().getTime();
      this.loyaltyToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.loyaltyToAdd);
      console.log(jsonData);
      
      const res = await LoyaltyService.addLoyalty(jsonData);
